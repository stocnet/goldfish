...
Example script prepared analyzing the Fisheries Agreements data with the `R` `goldfish` package. Models inspired by section 7 of:
...
V(startnet)$present <- startstates$present
V(startnet)$regime <- startstates$regime
V(startnet)$gdp <- startstates$gdp
startnet <- delete_vertices(startnet, V(startnet)$present == FALSE)
...
V(endnet)$present <- endstates$present
V(endnet)$regime <- endstates$regime
V(endnet)$gdp <- endstates$gdp
endnet <- delete_vertices(endnet, V(endnet)$present == FALSE)
