...
* **history**: Applies to `trans`, `cycle`, `mixed_trans`, `mixed_cycle`,
`mixed_common_sender`, and `mixed_common_receiver`.
In the default case of `history = "pooled"`, the effect returns change statistics
for all two-paths regardless of the order of creation of the ties in the two-path.
...
For `trans` and `cycle`, `history = "consecutive"` returns change statistics
for all two-paths when the subsequent tie is created directly after the preceding tie in the two-path.
For `mixed_*` effects, `"consecutive"` is not supported.
The argument order of the two networks defines the temporal sequence for `mixed_*` effects
with `history = "sequential"`: additions to the second network (`network2`) count
existing first-network neighbors, while additions to the first network (`network1`)
produce no new two-path entries. Removals always update the cache regardless of `history`.
...
† `mixed_*` effects support `"pooled"` and `"sequential"` only; `"consecutive"` is not supported.
...
On a **two-mode** focal network `indeg` is defined for `type = "alter"` -- the
receiver-side popularity, e.g. how many actors support a claim -- while
`type = "ego"` reads the sender side and is rejected.
...
On a **two-mode** focal network `outdeg` is defined for `type = "ego"` -- the
sender-side activity, e.g. how many claims an actor supports -- while
`type = "alter"` reads the receiver side and is rejected.
...
Where `attribute` is a column of a `global.goldfish` object created with
`make_global_attributes()`.
A global attribute is a time-varying scalar that applies equally to all actors,
not to individual nodes.
One `global()` call per attribute column is the intended usage; multiple columns
in the same `global.goldfish` object are linked as separate calls.
...
where $g(t)$ is the scalar value of the global attribute at time $t$.

**Initialization:** the statistic vector (DyNAM-rate) or matrix (REM) is
initialized by broadcasting $g(t_0)$ to all $n_1$ senders.
For REM, the diagonal is set to zero for one-mode networks.

**Update:** when a global event changes $g$ to $g'$, the statistic for every
actor is updated to $g'$:

* _DyNAM-rate:_ returns a 2-column change matrix `(node1, replace)` with one
  row per actor ($n_1$ rows total).
* _REM:_ applies `to_ego()` to expand the rate changes to a 3-column dyadic
  matrix `(node1, node2, replace)`, covering all ordered pairs except
  self-ties.

**Usage:** define a global attribute, link replace events to it, then
reference it in the formula with `global()`:

```{r, eval=FALSE}
seasons <- make_global_attributes(data.frame(winter = 0L, spring = 0L))
seasons <- link_events(seasons, season_events, replace = "winter")
m <- estimate(depEvents ~ global(seasons$winter), model = "DyNAM", sub_model = "rate")
```

Global attributes support replace-only events (no `increment` column).
Only the DyNAM-rate and REM models are supported; DyNAM-rate-ordered is
...
**Note:** For a node with no in-neighbors the aggregate is over an empty set and
so undefined. By definition the statistic then takes the mean of its defined
entries -- the average of the aggregate values over the nodes that *do* have
in-neighbors. This empty-neighborhood default is part of the effect's
definition, not missing-data imputation (see the *Missing data* section of the
`goldfish_data` help page for the separate attribute-imputation contract).
...
For a two-mode network $i$ and $j$ belong to different modes, so the attribute
is read on each side's mode (the sender-mode value against the receiver-mode
value). This effect is dyad-indexed and so is not available for the
DyNAM-rate submodel.
...
For a two-mode network the attribute is read on each side's mode (the
sender-mode value against the receiver-mode value).
...
_Note:_ For a node $j$ with no in-neighbors the statistic is undefined; by
definition it then takes the mean of its defined entries -- the average of the
similarities computed for the pairs $i$, $k$ over all $k$ that have at least one
in-neighbor. This is the effect's empty-neighborhood default, not missing-data
imputation.
...
covariate `network`.
It can be transformed by using `transformer_fn`.
This effect requires a **one-mode focal** network (the closure joins two nodes
of the same mode); the covariate `network` it reads may itself be two-mode. It
is not available for DyNAM-choice_coordination.
...
It can be transformed by using `transformer_fn`.
This is the canonical **two-mode** closure -- $i,j$ on one mode and $k,l$ on the
other -- so on a two-mode focal network set `is_two_mode = TRUE`. It is not
available for DyNAM-choice_coordination.
...
networks.
It can be transformed by using `transformer_fn`.
The two covariate networks may be two-mode, provided the chain conforms: the
receiver mode of `network1` must match the sender mode of `network2` (both are
the intermediate node $k$), and `network1`'s sender and `network2`'s receiver
must match the focal sender and receiver modes.
When `history = "sequential"`, the argument order of the networks defines the temporal
sequence: only ties added to `network2` ($k\rightarrow j$) create new two-path counts,
reflecting that the `network1` tie ($i\rightarrow k$) must already exist.
Ties added to `network1` produce no new counts under sequential history.
Removals update the cache regardless of `history`.
...
This effect cannot be used for two-mode networks and
DyNAM-choice_coordination.
When `history = "sequential"`, only ties added to `network2` ($j\rightarrow k$)
create new two-path counts, reflecting that the `network1` tie ($k\rightarrow i$)
must already exist. Ties added to `network1` produce no new counts.
Removals update the cache regardless of `history`.
...
This effect cannot be used for two-mode networks and
DyNAM-choice_coordination.
When `history = "sequential"`, only ties added to `network2` ($k\rightarrow j$)
create new two-path counts, reflecting that the `network1` tie ($k\rightarrow i$)
must already exist. Ties added to `network1` produce no new counts.
Removals update the cache regardless of `history`.
...
DyNAM-choice_coordination.
When `history = "sequential"`, only ties added to `network2` ($j\rightarrow k$)
create new two-path counts, reflecting that the `network1` tie ($i\rightarrow k$)
must already exist. Ties added to `network1` produce no new counts.
Removals update the cache regardless of `history`.
