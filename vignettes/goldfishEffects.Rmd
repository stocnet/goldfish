...
are thus available to be used with actor-oriented DyNAM models
\citep{Stadtfeld2017, Stadtfeld2017a} and tie-oriented
Relational Event Models \citep{Butts2008}.
...
* We omit the difference in the computation of the statistics when the 
  argument `isTwoMode` is used. In the default case whe 
...
\begin{align}
  r(i, t, x, z) &= 
  \sum_{j:~x(t)_{ji} > 0}{z(t)_j} / \sum_{j}{I(x(t)_{ji} > 0)}  && \text{ego type}\\
  s(i,j, t, x, z) &= 
    \sum_{k:~x(t)_{kj} > 0}{z(t)_k} / \sum_{k}{I(x(t)_{kj} > 0)} && \text{alter type}  
\end{align}
...
\begin{equation}
s(i,j, t, x, z) = |z(t)_i - \sum_{k:~x(t)_{kj} > 0}{z(t)_k} / \sum_{k}{I(x(t)_{kj} > 0)}|
\end{equation}
...
\emph{Note:} When the node $j$ does not have in-neighbors, its value is
...
Closure of two-paths (i->k->j) in the network through an event $i\rightarrow j$.
\begin{align}
...
Closure sender within 2 networks.
It is the tendency to create an event $i\rightarrow j$ when it closes more
two-paths with events ($k\rightarrow i$) in `network1` and
($k\rightarrow j$) in `network2` observed in the past events in the covariate
networks.
