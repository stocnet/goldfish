...
the definition is made once rather than once per kernel. The estimator's own
accumulation of the score SHALL be unchanged, and no coefficient may move: this
requirement governs where the stored diagnostic is computed, not the quantity
the optimizer follows.

Consolidating the event-loop engines onto the shared reduction replaces a
before/after difference of the running derivative with a direct evaluation of
the same quantity, which shifts stored values by a small amount. That shift is
an accepted consequence of having one implementation, **not** a precision
claim: the difference form's conditioning penalty was measured across the model
families at 1.2e-14 to 9.2e-13 relative, one hundred to eight thousand times
tighter than the cross-backend tolerance, so it is real in mechanism and
negligible in size.
