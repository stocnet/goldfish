...
- [ ] 0.1 Reproduce the crash on a **focal-less** hand-built `stocnet`: a
      ...
      crash sites.
- [ ] 0.2 Verify the frozen coefficient baselines PASS under `NOT_CRAN=true`
      before any change (the untouched-path reference).

## 1. The fix (design D1, D4, D5)

- [ ] 1.1 Stamp the resolved dependent as the working object's focal at
...
- [ ] 1.2 Guard the focal-name lookups (design D4): `ds_layer_map()` returns
      ...
      `ds_layer_map()`.
- [ ] 1.3 Confirm (or add) a clear error when **no** dependent is resolvable —
...
- [ ] 2.1 Cover the contract end to end: (a) focal-less two-mode object
...
- [ ] 3.1 Note in `?goldfish_data` / `add_info()` that `focal` is an optional
...
- [ ] 3.2 Reframe the vignettes to lead with the focal-less pattern (formula/
...
- [ ] 3.3 Reframe the roxygen `@examples` that set `focal =` as required
...
- [ ] 4.1 Full `NOT_CRAN=true` suite green, frozen baselines PASS **not** SKIP;
      `lintr::lint_package()` clean on touched files; `devtools::document()`;
      `openspec validate formula-drives-focal --strict`.
- [ ] 4.2 `NEWS.md` bullet: a hand-built `stocnet` with no `info$focal` now
