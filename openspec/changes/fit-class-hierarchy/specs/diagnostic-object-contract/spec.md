...
recorded verdict in the fit-class contract table for every concrete fit class,
so that what a flavored or Monte-Carlo fit returns from a diagnostic is stated
rather than implied by which method happens to have been written. A diagnostic
...
- **THEN** each call either returns the diagnostic, or aborts with the reason
  the table records, and no combination is left to whichever method happens to
  exist
