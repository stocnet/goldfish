...
- [ ] 1.1 Confirm the predecessors' state: `class-naming-scheme` has landed
...
- [ ] 1.2 Enumerate every generic dispatching on a fit class and every concrete
...
- [ ] 1.3 Choose the table's on-disk form (design D5): machine-readable, and
      the same artifact the test reads and a human edits.

## 2. The logLik row first

- [ ] 2.1 Decide and record the `logLik` verdict for a Monte-Carlo fit
...
- [ ] 2.2 Write the refusal message if the verdict is `refuse`: name the class,
...
- [ ] 3.1 Introduce the parent class on the fit constructors. It lands in the
      same commit as the table (design D1) — never before it.
- [ ] 3.2 Fill the remaining cells. Every `override` records its reason; every
      `refuse` records the reason its message states.
- [ ] 3.3 Add the completeness test (design D4): read the table, enumerate
...
- [ ] 4.1 Give `summary`, `tidy`, `glance` and `print.summary` their verdicts,
      so flavored fits stop silently lacking them.
- [ ] 4.2 Decide `coef_layout`, currently flavored-only: `inherit` for the
      single class, or a deliberate `refuse` (design, Open Questions).
- [ ] 4.3 Tests for each newly reaching generic on a flavored fit; snapshots
      for every `refuse` message, reviewed individually.
- [ ] 4.4 Verification: `air format` → `lintr` → **not-cran-test**.

## 5. Close

- [ ] 5.1 The completeness test passes with no undecided cell, and adding a
      throwaway generic makes it fail — verify the test can actually fail.
- [ ] 5.2 `devtools::check()` clean; `openspec validate fit-class-hierarchy
      --strict`; `bash .plan/opsx-spec-placement-check.sh fit-class-hierarchy`.
- [ ] 5.3 `NEWS.d/` fragment: flavored fits gain `summary`/`tidy`/`glance`;
...
- [ ] 5.5 Update ADR-0038's `spec:` field to this change, and note in
