...
- [x] 9.1b Establish the blast radius of the flavored dependent-events filter
...
- [x] 9.1c Carry each process's own flavor to the **recording** call so
      `stocnet_dependent_events()` filters the table it records (D25). Task 9.1b
      established that the obvious implementation is wrong: `modeled_flavor` is
      not a filter argument but the single-flavor path's whole restriction
      mechanism, with five consumers — it filters the dependent event stream
      itself (`R/event_streams.R:215`), the parse, the alias resolution and the
      constraint derivation. The flavored path restricts by a different
      mechanism (`preprocess_flavored()` writes a fid-indexed list already split
      per flavor), so setting `spec$modeled_flavor` there would double-apply the
      restriction against an already-split preprocessed object. Pass the flavor
      to the recording site at `R/model_estimate.R:2687` **only**, leaving
      `:1681`/`:1912`/`:2272` on the NULL they get today.
- [x] 9.1d Test that each process records only its own events — the recorded
