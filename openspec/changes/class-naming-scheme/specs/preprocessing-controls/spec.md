...
`c("preprocessing_goldfish", "list")`. The `print` method SHALL dispatch on
...
- **THEN** the returned object inherits `preprocessing_goldfish`, carries
...
"preprocessing_goldfish")`. Functions minted after this change that take
...
`preprocessed_goldfish` object to skip preprocessing, with exactly the
...
- **WHEN** a `preprocessed_goldfish` object is passed as
...
- **WHEN** a `preprocessed_goldfish` with an outdated format version is
