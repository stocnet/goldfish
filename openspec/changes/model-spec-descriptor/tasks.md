...
- [x] 1.1 Confirm `class-naming-scheme` is flagged **landed** before starting
...
- [x] 1.1a Note the one predecessor artifact this change will turn red:
      ...
      next real one.
- [x] 1.2 Inventory: for each of the 9 variants record its current class
...
- [x] 1.3 Build the descriptor in the spec constructor, subsuming the existing
...
- [x] 2.2 Route the three DyNAM-i `preprocess.*` methods through
      ...
      Non-Goals).
- [x] 2.3 Tests: preprocessing output is byte-identical to before for every
      variant. Compare against stored output, not against a re-run of the new
      code.
- [x] 2.4 Retire the `dyad_symmetric` axis string (design D1a): one-mode
      ...
      changing the string.
- [x] 2.5 Verification: `air format` → `lintr` → **not-cran-test**. The
...
- [x] 2a.1 Inventory the five shapes: the three `compute_statistics()` output
...
- [x] 2a.2 Return one `goldfishStat` class from every shape, with `storage`
...
- [x] 2a.3 Fold the four retired classes' behaviour into one `print` method
      ...
      (design D9).
- [x] 2a.4 Tests: each output shape carries the class and the right `storage`;
      the gather shape is classed; a flavored and a single result differ only
      in `scope`; print renders all shapes.
- [x] 2a.5 Verification: `air format` → `lintr` → `devtools::document()` →
      **not-cran-test**.

## 2b. Collapse the mechanical flavored fan-outs (design D10)

- [x] 2b.1 Confirm the classification against the tree before editing: the six
...
- [x] 2b.2 Move `coef` and `vcov` onto `flavored_component_apply()` and reduce
...
- [x] 2b.3 Tests: each of the six returns per-component results named as
      before, for both a flavored and a single fit; snapshots reviewed, not
      accepted wholesale.
- [x] 2b.4 Verification: `air format` → `lintr` → `devtools::document()` →
      **not-cran-test**. Coefficients cannot move — these are fan-outs, not
      arithmetic.

## 3. One name per behavior — retire the flag pair

- [x] 3.1 Replace `right_censored` / `intercept_scalars` with `timing`
...
- [x] 3.2 **Rewrite, do not swap, every comment and roxygen line that
      ...
      forbidden.
- [x] 3.3 Grep for both retired names and require an empty result outside the
...
- [x] 4.1 Carry a likelihood class on the spec (design D3) and re-point
      `compute_event_contribution` dispatch at it; variants sharing an
      implementation share the class.
- [x] 4.2 Delete the three DyNAM-i alias assignments (design D4). Assert that
      a DyNAM-i rate spec dispatches to the DyNAM rate method and that no
      alias is registered.
- [x] 4.3 Confirm `estimate_int` still dispatches on the axis and that
      preprocessing no longer dispatches per variant.
- [x] 4.4 Verification: `air format` → `lintr` → **not-cran-test**. This is
      the task most able to move a coefficient; a moved baseline stops it
      (ADR-0021), it is not re-frozen.

## 5. Re-derivation sweep

- [x] 5.1 Replace the 43 `model`/`sub_model` behavior branches outside the
...
- [x] 5.2 Keep `model` / `sub_model` as provenance (design D5): print
      methods and error messages still report what the user asked for.
- [x] 5.3 Add the guard test (design D6) failing on any `model`/`sub_model`
...
- [x] 5.4 Verification: `air format` → `lintr` → **not-cran-test**.

## 6. Close

- [x] 6.1 The guard test passes and the descriptor covers every variant in
      the 1.2 table.
- [x] 6.2 `devtools::check()` clean; `openspec validate model-spec-descriptor
      --strict`; `bash .plan/opsx-spec-placement-check.sh model-spec-descriptor`.
- [x] 6.3 Write a `NEWS.d/` fragment (internal-refactor note; no user-visible
...
- [x] 6.4 Final **not-cran-test** on the full suite; frozen 1e-6 and C++
      goldens PASS not SKIP.
- [x] 6.5 Re-run the placement pre-flight for `parametric-rates` and
