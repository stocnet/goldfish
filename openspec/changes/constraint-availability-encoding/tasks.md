...
- [x] 1.1 Detector first: an ego-kind constraint on a DyNAM choice model
...
- [x] 1.2 Capture the before-state of the reference fit in
...
- [x] 1.3 Implement the outer branch in `fold_active_dyad_support()`: two factor
      vectors, cell `(i, j) = active_sender[i] & active_dyad[j]`. The
      unconstrained REM path already produces this encoding and the engines
      already consume it.
- [x] 1.4 Move the expectation at `test-support_constraint_ego_fold.R:65` from
...
- [x] 1.5 Rewrite the comment above the fold, by hand, so it describes the
      ...
      behind.
- [x] 1.6 Verification: `NOT_CRAN=true`, baselines PASS not SKIP; record the
      ego-kind availability object's size before and after.

## 2. The choice family's missing sender warning (design D3)

- [x] 2.1 Detector first: an ego-kind constraint on a choice model with a
      gated-out, never-observed sender warns once and names the node. It is
      silent today.
- [x] 2.2 Add the sender-side accumulation to `validate_support_constraint()`'s
      ...
      the observed events.
- [x] 2.3 `r-lib:cli`: semantic elements, data interpolated rather than literal
      markup, pluralisation handled, and a pinned reproducible cli context for
      the snapshot.
- [x] 2.4 Tests: the four existing fail-fast validations are unchanged in
...
- [x] 2b.1 Detector first, and TWO fixtures, because Fisheries alone cannot
...
- [x] 2b.2 Make `fold_active_dyad_support()` stash the RAW receiver crossings
      ...
      one-line edit.
- [x] 2b.3 Read the receiver presence per event in the choice branch through
...
- [x] 2b.4 Decide and record what "present" means for the warning's denominator
...
- [x] 2b.5 Tests: the four existing fail-fast validations keep their messages
...
- [x] 2b.6 Verification: `NOT_CRAN=true`, baselines PASS not SKIP. Note in
...
- [x] 3.1 Answer the design's open question: does an ego-kind constraint on REM
      ...
      choice fold.
- [x] 3.2 `NEWS.d/` fragment: the encoding under Internal, the new warning as a
      user-facing bullet, since a new condition is something a user sees.
- [x] 3.3 `bash .plan/opsx-spec-placement-check.sh constraint-availability-encoding`
      and `openspec validate constraint-availability-encoding --strict` clean.
- [x] 3.4 Final verification: full `NOT_CRAN=true` suite green, baselines PASS
      not SKIP.
