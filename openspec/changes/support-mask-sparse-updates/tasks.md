...
- [ ] 1.1 Write `.plan/sp/constraint_baseline.R`, a re-runnable grid recording
...
- [ ] 1.2 Record the grid in `.plan/sp/constraint_baseline_2026-09.md` and in
      ...
      under `~ indeg(msg) < 20`.
- [ ] 1.3 Record the atom-pool count per substrate: how many atom maintainers a
...
- [ ] 1.4 Verification: `NOT_CRAN=true` suite green at the branch start, frozen
...
- [ ] 2.1 **One layer, rate and choice, one mask.** A specification carrying both
      ...
      row reduction.
- [ ] 2.2 **Creation and deletion flavors with a state constraint plus an
...
- [ ] 2.3 **Mixed-kind atoms.** A constraint combining a scalar atom, an
...
- [ ] 2.4 **Composition change under constraint.** Nodes entering and leaving
...
- [ ] 2.5 **No per-event dense list.** Assert directly the living-spec scenario
...
- [ ] 2.6 **The incremental mask equals the from-scratch mask.** Over a full
...
- [ ] 2.7 Verification: run the new fixtures and record which fail and why, in
...
- [ ] 3.1 Extract the kind-shaped vocabulary: `kind_length()`,
...
- [ ] 3.2 Route interaction operands on the dyad branch through the core, stored
...
- [ ] 3.3 Tests: an alter operand's update writes one vector entry and builds no
...
- [ ] 3.4 Verification: `NOT_CRAN=true`, frozen baselines and C++ goldens PASS
      not SKIP. Record the interaction-model timings against the group 1 grid.

## 4. Constraint atoms at their kind (design D3)

- [ ] 4.1 Store each atom at `atom_kinds[gid]` and write it in place, removing
      `expand_operand_update()` from the atom path and the per-event copy of the
      atom matrix.
- [ ] 4.2 Remove the `<<-` state write in `apply_atom_event()`, which is
...
- [ ] 4.3 Tests: `tracemem` reports no duplication of the atom buffers or the
...
- [ ] 4.4 Verification: fixtures 2.3 and 2.6 go green; the folded availability
      objects stay byte-identical; baselines PASS.

## 5. The mask as a stream (design D4, D5)

- [ ] 5.1 Implement mixed-kind elementwise evaluation: project each atom into the
...
- [ ] 5.2 Emit the mask as an initial value plus a flat update buffer with a
      per-event pointer and its kind, replacing the snapshot list produced by
      `preprocess_support_mask()`.
- [ ] 5.3 Maintain the DyNAM-rate sender gate as the per-sender allowed-receiver
...
- [ ] 5.4 Fold the availability incrementally from the mask's flip stream,
...
- [ ] 5.5 Tests: fixtures 2.1, 2.4, 2.5 and 2.6 go green; a symmetrised
...
- [ ] 5.6 Verification: `NOT_CRAN=true`, baselines PASS. Re-measure the group 1
...
- [ ] 6.1 Compile the constraint once per `(layer, flavor)` process rather than
      once per sub-model family, and have every sub-model of that process read
      the shared maintained mask.
- [ ] 6.2 A constraint whose atoms are all sender-axis maintains no dyad mask:
      its kind is ego or scalar and the gate is the mask itself.
- [ ] 6.3 Tests: fixtures 2.1 and 2.2 assert one maintainer and one evaluation
      stream per process; task 1.3's counts halve on both substrates.
- [ ] 6.4 Verification: `NOT_CRAN=true`, baselines PASS; the merged walk's
      constrained ratio re-measured against `preprocess-one-walk` task 0.9's
      1.16.

## 7. Atoms as operands of the main plan (design D8)
...
- [ ] 8.1 Rewrite `validate_support_constraint()` to advance the flat stream
      instead of indexing `support[[e]]`. It is already a sequential per-event
      walk, so this is a change of source, not of logic.
- [ ] 8.2 Remove `support_mask$support` from the preprocessed object, gated on
      `prep_version`.
- [ ] 8.3 Tests: the fail-fast validations (empty risk set, observed dyad
      ...
      size is reported.
- [ ] 8.4 Verification: `NOT_CRAN=true`, baselines PASS.

## 9. Report

- [ ] 9.1 Re-run the group 1 grid and tabulate before against after for every
...
- [ ] 9.2 Record what the change does NOT fix, with numbers: whatever remains of
      the constrained-versus-unconstrained gap, and where it now sits.
- [ ] 9.3 `NEWS.d/` fragment under Internal (ADR-0040; no `NEWS.md`, no Version
      bump on a feature branch).
- [ ] 9.4 Update `preprocess-one-walk` task 0.9's write-up to correct its stated
