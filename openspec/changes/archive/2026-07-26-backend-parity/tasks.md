...
- [x] 3.5 **Pre-existing bug in the `cpp` timed-rate rank path, unmasked by
      3.4's guard fix.** RESOLVED — and **not in C++ at all**. No debugger was
      needed: the second NA guard was the culprit, in R.

      **Diagnosis.** `estimate_c_int()`'s Newton loop decides step acceptance
      with `!any(is.na(unlist(res)))` — the *same* scan 3.4 taught the
      initial-parameters guard to skip `observed_rank` with, but a second site
      that never got the exclusion. With ranks requested on a model carrying a
      right-censored interval, `res$observed_rank` holds by-design NAs, so
      **every** step is rejected as a numerical failure. The first rejection
      restores `informationMatrix.old`, which is initialized to `NULL`; then
      `NULL[idUnfixed, idUnfixed]` returns `NULL` (silently — R does not error
      on that), `solve(NULL)` is a `try-error`, and the loop reports "Matrix
      cannot be inverted; probably due to collinearity between parameters."
      That is why the symptom named the wrong subsystem and why the rank block
      in `DyNAM_rate_default.cpp` — the documented place to look — was innocent.

      **Fix.** Both step-acceptance guards call `has_unexpected_na()`, the
      shared exclusion list 3.4 introduced: `cpp_interface.R` and
      `estimation_core.R`. The `r` backend had the identical latent defect and
      would have hit it the moment task 4.1 gives it ranks; fixed in the same
      commit rather than left as a trap. `stepAccepted` -> `step_accepted`
      while both lines were open (camelCase-on-touch policy).

      **Regression test.** `dataTest` + `indeg(networkExog)` — an effect on an
      exogenous network is what produces right-censored intervals (4 of 16
      here), the same structural cause as the Social Evolution repro but on the
      small fixture. Asserts a *free* fit (not `max_iterations = 0`, which
      breaks out before step acceptance) reaches the same coefficients with and
      without ranks, on `cpp` and `gather`. Verified to fail before the fix.

      **Note on the existing 3.4 test:** it runs `~ 1 + indeg` on `dataTest`,
      which preprocesses to **zero** censored intervals — so both of its
      censoring assertions were passing vacuously and it could never have
      caught this. The new test carries the censored-interval requirement
      explicitly.
...
- [x] 3.6 Gather Poisson kernel stores `NA_real_` for the conditional
      component on right-censored intervals (D21), replacing the placeholder
      `lin_pred[selected] − lse` it currently stores there, matching the `r`
      backend. Own commit; `cpp-recompile` before testing. Tests: on a
      fixture **asserted** to carry ≥ 1 right-censored interval after
      preprocessing, the gather conditional vector is NA at exactly the
      censored positions and agrees with `r` elsewhere. Sequenced **before**
      4.1 resumes, so the tree never carries a cross-backend divergence on a
      stored primitive.

## 4. The r backend

- [x] 4.1 `r` backend accumulates `ranks` and `margins` in its contribution loop
...
- [x] 4.2 Rewrite the two existing parity tests that reconstruct `r` ranks and
...
- [x] 5.1 `cpp` **and `gather`** return per-event probabilities natively, ending
      the `return_probabilities` redirect onto `r` — defined on every family per
      D16 (exact-time: next-event probabilities, so DyNAM-rate gains the
      primitive) — and the two exact-time `*_default.cpp` kernels gain the
      ...
      **Scope amended 2026-07-26 (D22), after reading ahead from 4.2 found two
      gaps the original text did not cover — both decided with the user:**

      1. **`gather` is in scope, not just `cpp`.** The three
         `compute_*_selection.cpp` kernels form the probability vector
         internally and discard it, exactly as `cpp` does. The redirect this
         task removes fires for *every* backend that is not `r`, and D16 says
         `cpp` **and** `gather` produce the primitive natively — leaving gather
         out would force 5.2's capability table to carry an unsupported cell,
         against the change's central goal. All nine kernels gain the flag.
      2. **The two exact-time `cpp` kernels also gain `conditional_logl`**
         (D17), which they have never computed — gather
         (`compute_poisson_selection.cpp`) and `r` (`estimation_core.R`) are
         its only producers today, and `cpp_interface.R` surfaces it only when
         present. 5.3 requires comparing it across backends directly, so it
         must exist on `cpp` first. Folded here rather than into its own task
         because 5.1 already opens those two frozen-baseline kernels for the
         margins accumulator, and opening them twice doubles the baseline
         exposure for no bisectability gain.

      **The conditional is computed from an lse added beside the raw
      normalizer, not from `log(normalizer)`** (D22): the exact-time `cpp`
      kernels run the raw `exp` pass, and D4's Non-Goal keeps the *likelihood*
      on it. The conditional is a ratio, so it takes the D19-exact
      `x_obs − lse` form and matches gather bit-for-bit at 5.3's 1e-10. The
      raw pass and every quantity derived from it are untouched, so no frozen
      coefficient can move. Rejected: `x_obs − log(normalizer)` (diverges from
      gather exactly where the rates over/underflow — the silently-wrong case
      D19 documented); a single shifted pass like 3.2c gave gather (cleanest,
      but rewrites a frozen kernel's likelihood arithmetic, against D4).

      **Shape contract settled first (D23).** The three backends disagreed on
      how `probabilities` is indexed — `r` stores the reduced risk set (13
      distinct lengths, 137–151, across the 215 events of `fish_dynam_rate`;
      and length 3 of 4 on `dataTest` DyNAM-choice, where the reflexive
      receiver is dropped rather than zeroed), while `cpp` / `gather` have the
      full masked actor set in hand. All three move to the **full-length,
      actor-indexed** vector with 0 on disallowed alternatives, matching how
      `margins` is already stored everywhere. This makes `r`'s existing output
      shape a BREAKING change (NEWS, 7.4).

      **Split for bisectability** (nine kernels and an R-side shape change is
      far past one reviewable diff; the same D7 reasoning that split 3.2 / 3.3
      per kernel family):

      - [x] 5.1a `r` stores full-length actor-indexed probabilities, scattering
            through the `sender_slots` / `receiver_slots` it already uses for
            margins. Pure R, establishes the contract every later step is
            written against. Tests: shapes per family; a composition-change
            fixture where the length is constant across events and equals the
            actor count; zeros exactly at the absent / disallowed positions.
      - [x] 5.1b The six `*_default.cpp` kernels gain `return_probabilities`;
            the two exact-time ones additionally gain the probability-scale
            margins accumulator (D12) and `conditional_logl` from an added lse
            (D22). `cpp-recompile` before testing; frozen baselines PASS.
      - [x] 5.1c The three `compute_*_selection.cpp` kernels gain
            `return_probabilities`, scattering through the `index_i` /
            `index_j` task 3.1 already threaded into the dispatch.
      - [x] 5.1d Remove the `return_probabilities` backend redirect in
            `model_estimate.R` and surface the new component through
            `cpp_interface.R`. Tests: a `cpp` fit and a `gather` fit carry
            probabilities and emit no backend-substitution warning; per-event
            sums are 1 (2 for coordination, D23); the guardrail still fires.
- [x] 5.2 The `(backend, primitive)` support table plus the single check in
...
- [x] 5.3 Full three-way parity suite at a **fixed** parameter vector
      ...
      risk set, not within a CSR group). Coordination-fixture caveat from the
      4.1 findings (progress.md): rank comparison must reduce in **log-space**
      (product-space `P(i→j)·P(j→i)` underflows into spurious exact ties), and
      near-degenerate dyads can still tie-flip between R's `stable_softmax`
      logZ and cpp's `log_sum_exp` — choose/assert a fixture with no
      cross-implementation tie blocks so "ranks exact" stays exact, rather
      than weakening the comparison. Then: ranks exact, margins (both exact-time
      scale variants, D12; coordination margins are single-sided over
      unordered dyads, crediting both members — the fourth geometry) / scores /
      ...
      over dependent events, with NA required at exactly the right-censored
      positions on every backend, D21)
...
- [x] 5.4 Iteration-path diagnostics smoke (D21): per backend, a fixture
      **asserted** to carry ≥ 1 right-censored interval after preprocessing,
      every primitive the backend supports requested, `max_iterations ≥ 2` —
      estimation completes, the by-design NAs pass the shared
      `has_unexpected_na()` guard at *step acceptance* (the site the
      Newton-loop bug lived in; `max_iterations = 0` never reaches it), and
      the fit carries the primitives. De-vacuize the 3.4 regression test the
      same way: it currently passes on a fixture that preprocesses to zero
      censored intervals — add the precondition assertion so it fails loudly
      if the fixture goes vacuous again. 5.3 stays fixed-θ on purpose; this
      task owns loop mechanics.

## 6. Fold the frozen engines onto the shared header (last, design D7)

- [x] 6.1 Move the six `*_default.cpp` engines onto `event_reductions.h`,
...
- [x] 6.2 If 6.1 perturbs a baseline, stop and record the finding rather than
...
- [x] 7.1 Enforce the revised D6 division: `residuals-gof` carries **no**
...
- [x] 7.2 Update `residuals-gof` where this change removes an assumption it was
...
- [x] 7.3 Freeze a gather coefficient baseline (D15), after the section-6 fold
      ...
      archive.
- [x] 7.4 NEWS entry: all five primitives on all three backends; the removed
