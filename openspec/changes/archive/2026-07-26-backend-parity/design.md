...
### D21 — By-design NA on right-censored intervals: one contract, one guard (2026-07-25)

Per-event primitives that are undefined on a right-censored interval store
`NA_real_` there, identically on every backend: `observed_rank` (no observed
alternative to rank) and the conditional loglik component (no mover, so
`log p_obs` is undefined — the same fact that excludes censored intervals
from the probability-scale margins, item 4 of the Appendix, by a different
mechanism). The gather Poisson kernel currently violates this — it stores
`lin_pred[selected] − lse` unconditionally, where `selected` is a
placeholder on censored intervals (measured: all four censored intervals on
the `dataTest` + `indeg(networkExog)` fixture carry finite, meaningless
values) — and is brought to match the `r` backend in its own kernel commit
(task 3.6) before 4.1 resumes, so no commit leaves two backends disagreeing
on a stored primitive.

The guard rule is the load-bearing half: **`has_unexpected_na()` is the
only place the by-design-NA exclusion list lives.** The Newton-loop bug
that motivated this decision was exactly a second scan site — step
acceptance gated on `!any(is.na(unlist(res)))` while only the
initial-parameters guard had been taught the `observed_rank` exclusion; the
first "failed" step restored a still-`NULL` `informationMatrix.old`, and
`NULL[i, j]` being silently `NULL` turned it into the misleading
`solve(NULL)` "Matrix cannot be inverted". Any future per-event NA scan
MUST go through the shared guard, never an ad-hoc `is.na` sweep.

Consequences for verification: identities and cross-backend parity for
these components are asserted over dependent events, with NA required at
exactly the censored positions on every backend; and any test exercising
censored-interval behavior asserts its fixture precondition (≥ 1
right-censored interval after preprocessing) so it cannot pass vacuously —
the 3.4 regression test did (its fixture preprocesses to zero censored
intervals), and `max_iterations = 0` exits before step acceptance, which
together is why the bug was reachable only in the field. The
iteration-path coverage lands as task 5.4; 5.3 stays fixed-θ on purpose
(its job is numerical parity, not loop mechanics).

### D22 — The probabilities task covers all nine kernels, and carries the cpp conditional component (2026-07-26)

Reading ahead from task 4.2 found two gaps between what section 5's text says
and what the tree contains. Both were settled with the user rather than
implemented against the stale text.

**Gather is in scope for `probabilities`.** Task 5.1 named only the `cpp`
backend, but the three `compute_*_selection.cpp` kernels are in the same
position as the six `*_default.cpp` ones: they form the probability vector
(`compute_multinomial_selection.cpp:122` and the Poisson kernel's equivalent)
and store nothing. The redirect 5.1 removes fires for *any* backend that is not
`r`, and D16 already states that `cpp` **and** `gather` produce the primitive
natively. Omitting gather would make 5.2's capability table carry an
unsupported cell for a primitive that is mathematically defined there and
already computed there — the exact shape of failure this change exists to end.
So all nine kernels gain the flag, in one task rather than two: the storage
plumbing and the shape contract are the same work on both sides, and splitting
them would leave an intermediate commit where `probabilities` is supported on
two backends out of three with no table to describe it.

**`conditional_logl` does not exist on `cpp`, and is built here.** D17 specifies
the Cox partial-likelihood component as kernel-computed, and gather
(`compute_poisson_selection.cpp`) and `r` (`estimation_core.R`) both produce
it — but neither `DyNAM_rate_default.cpp` nor `REM_default.cpp` ever has, and
`cpp_interface.R` surfaces it only when the kernel returns it, so its absence
on `cpp` has been silent. Task 5.3 requires the conditional "compared across
backends directly", which cannot pass until `cpp` has one. It folds into 5.1
because 5.1 already opens those two kernels for D12's probability-scale margins
accumulator; a separate task would open a frozen-baseline path a second time
and double the regression exposure for bisectability this change does not need
(the conditional is a new opt-in output that no existing quantity reads).

**The conditional uses an added lse, while the likelihood keeps the raw pass.**
This is the substantive half. The exact-time `cpp` kernels compute the raw
`normalizer = Σ exp(x_j)`, and D4's Non-Goal is explicit that the timed
*likelihood* must keep it — shifting there would be a different model, not a
stabilization. The conditional, however, is a ratio (`log p_obs`), so the shift
cancels exactly, and D19 measured what the naive form costs: at `max(x) = −745`
the raw ratio returns `1` where the truth is `0.705`, silently. So these two
kernels compute a log-sum-exp *beside* the raw normalizer and take
`log p_obs = x_obs − lse`, which is what gather already does — making the two
backends bit-comparable at 5.3's 1e-10 rather than agreeing only where nothing
overflows. Nothing the raw pass feeds is touched, so no frozen coefficient can
move.

Rejected: `x_obs − log(normalizer)` from the existing raw sum — no extra pass,
but it diverges from gather precisely in the regime the diagnostic is for.
Rejected: giving these kernels the single shifted pass 3.2c gave gather's
Poisson kernel — numerically the cleanest and it would unify the two
implementations, but it rewrites a frozen-baseline kernel's likelihood
arithmetic, which is what D4's Non-Goal and D7's sequencing both exist to
prevent.

### D23 — `probabilities` is actor-indexed and full-length on every backend (2026-07-26)

D16 defines the primitive's *value* (`p_e = w_e / sum(w_e)`, summing to 1 per
event) but never its *indexing*, and the three backends disagree today. Measured
before deciding:

- `r` stores the **reduced** risk set. On `fish_dynam_rate` (fisheries, the one
  baseline fixture with composition change) the 215 per-event vectors have 13
  distinct lengths, 137–151. Reduction is not only about composition: on
  `dataTest` with 4 actors and no presence changes, DyNAM-choice stores length
  **3**, because the reflexive receiver is dropped rather than zeroed.
- `cpp` and `gather` work over the full actor set with an `allowed` mask, so
  full-length output is what their loops already have in hand.

So parity for this primitive was undefined, and 5.3 could not have asserted it.

**Decision: every backend stores the full-length, actor-indexed vector, zero
where the alternative was not in the risk set** — length `n_actors_1` on the
sender axis, `n_actors_2` on the receiver axis, `n_actors_1 × n_actors_2` on the
dyad axis. Position *is* the actor id, at every event, on every backend.

Grounds: it is the only shape in which the primitive is usable downstream
without replaying the presence history — under the reduced shape, index 3 is a
different actor at different events, so `residuals-gof`'s per-actor consumers
could not read it. It also matches `margins`, which is already full-actor-length
on all three backends (`assemble_r_margins()` scatters the reduced accumulators
through `sender_slots` / `receiver_slots` for exactly this reason), so the two
primitives stop disagreeing about what an index means. And it makes 5.3's
comparison a plain elementwise check at 1e-10 rather than an alignment problem.

Costs, accepted: the `r` backend's existing `eventProbabilities` output changes
shape — an observable **BREAKING** change, taken at 2.0.0 with a NEWS entry,
in a primitive this change is re-specifying anyway; and per-event storage grows
when many actors are absent, which the existing
`warn_probabilities_footprint()` guardrail already sizes and warns about.

Unchanged by this decision: coordination's per-event matrix sums to **2**, not
1, because each unordered dyad's probability is credited to both `(i,j)` and
`(j,i)` — the existing symmetric-matrix semantic of `getLikelihoodMM()` and
`DyNAM_MM_default.cpp`. D16's "sums to 1" is read per *unordered dyad* there.
This change preserves the semantic rather than renormalizing it; the fourth
geometry noted in D14 and the 4.1 findings applies here too.
