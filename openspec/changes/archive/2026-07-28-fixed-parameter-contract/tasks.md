...
- [x] 1.1 `fixed_spec` constructor + validation in `formula_validate.R`:
...
- [x] 1.2 `offset(term, coef =)` in the parser + assembler (design D7):
...
- [x] 1.3 The name resolver + `initial_spec` (design D6): one internal
...
- [x] 1.4 The shared masking helper (estimation-side, design D2): takes
...
- [x] 2.1 `estimate_c_int()` consumes the helper: duplicated decode block
...
- [x] 2.2 `estimate_int()` consumes the helper: duplicate block
      (estimation_core.R:84-114) deleted. Cross-backend offset fixture
      agreement unchanged (the every-backend spec scenario).
- [x] 2.3 `model_estimate.R` pass-through: the estimator front-end hands
...
- [x] 2.4 The flavored value channel (designs D6/D7): offsets come from
...
- [x] 3.1 `GetDetailPrint()` returns the typed per-term data.frame (design
...
- [x] 4.1 Coordination note into `residuals-gof` task 2.1 (its
