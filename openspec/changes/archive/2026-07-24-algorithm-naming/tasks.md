...
`lintr` runs on them, `NOT_CRAN=true` baseline PASS before each commit,
...
- [x] 1.1 Rename `set_estimation_opt()` → `set_algorithm_newton()` in
...
- [x] 1.2 Rename `set_preprocessing_opt()` → `set_preprocessing()`: class
      `c("preprocessing.goldfish", "list")`, roxygen + `print` method
      migrated; `devtools::document()`.
- [x] 1.3 Add soft-deprecated alias wrappers `set_estimation_opt()` /
      ...
      hierarchy asserted.
- [x] 1.4 Re-point every live deprecation and cli message off the old
...
- [x] 2.1 `estimate_dynam()` / `estimate_dynami()` / `estimate_rem()`:
      ...
      inherited elsewhere).
- [x] 2.2 Migrate all internal callers and the test suite to the new
...
- [x] 3.1 Rename `examine_outliers()` → `diagnose_outliers()` (with
...
- [x] 4.1 Sweep vignettes, README, and `_pkgdown.yml` references to the
      renamed surfaces; regenerate affected man pages; confirm no
      `\arguments` inheritance broke.
- [x] 4.2 NEWS.md 2.0.0 naming-migration section with the complete old→new
...
- [x] 5.1 residuals-gof artifacts: `keep_preprocessed` →
...
- [x] 5.2 abmcem + dynes-augmentation artifacts — sweep ALL old names:
...
- [x] 5.3 make-multivariate-spec artifacts: `evaluate_engine` →
