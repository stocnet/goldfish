...
### D7 — Drop the auto-ordinal `rate` resolution (user decision 2026-07-23)
Applying D4 surfaced a decision the spec cannot make cleanly: a no-intercept
`sub_model = "rate"` formula was silently auto-converted to the ordinal
(`rate_ordered`) spec, yet the estimation family guard keyed on the RAW
`sub_model` string, so the auto-converted `"rate"` path and an explicit
`"rate_ordered"` path — which produce byte-identical specs — got OPPOSITE
constrained-estimation decisions (abort vs run). No spec/descriptor read can
reproduce both, because the specs are identical. Resolution: retire the
auto-ordinal conversion entirely. `sub_model = "rate"` ALWAYS models the
waiting times between events; a rate formula without an explicit time
intercept gets the intercept added (the baseline hazard the waiting-time
likelihood needs) with an informative message, rather than collapsing to the
order-only partial likelihood. Ordinal (order-only) modeling is requested
explicitly with `sub_model = "rate_ordered"`. Consequences: the three
auto-ordinal sites in `model_estimate.R` (the `spec_sub_model`/
`validity_sub_model` switches and the DyNAM deprecation warning) are removed;
`rem_rate_ordered_spec` / `dynam_rate_ordered_spec` are reachable ONLY via
explicit `"rate_ordered"`, so the capability map keys cleanly on the spec
class (every recipe family supported; DyNAMi unsupported). The frozen
coefficient baselines are unaffected — they use explicit `1 + ...` for rate
and explicit `"rate_ordered"` for ordinal, never the auto-ordinal path.
Non-baseline tests that asserted the old auto-conversion (deprecation
warnings, the ordinal-REM "not yet consumed" abort) are updated deliberately.

### D8 — Descriptor gains a `normalizer` field (implementation, §4.3)
Converting the compiled interface's `modelTypeCall` branches to descriptor reads
needed one distinction the geometry fields (axis/fold/encoding/symmetrize) do
not carry: the likelihood normalizer (Poisson timespan vs multinomial softmax vs
the mutual coordination product). The axis alone cannot tell `rate` (Poisson)
from `rate_ordered` (multinomial) — both are sender-indexed. So the descriptor
gains `normalizer` ("poisson" | "multinomial" | "coordination"), derived at
construction from `sub_model` (`rate`→poisson, `choice_coordination`→
coordination, else multinomial). `estimate_c_int`'s timespan/intercept-init
branches and the `compute_` kernel selection read it; `estimate_` selects the
C++ estimator by spec class (`inherits`); `gather_` reads `axis` +
`normalizer`. All byte-identical — the same estimator/kernel/gather is chosen,
keyed on spec data instead of a model-type string.
