...
- [x] 0.1 Confirm `backend-parity` is archived and its deltas are folded into
...
- [x] 0.2 Verify the node-lookup completeness D3 depends on before anything is
...
- [x] 1.1 Checksum guard on the frozen baselines (D6): a test asserting the
      ...
      omission.
- [x] 1.2 `parallel` into `Suggests` (D8) with `requireNamespace()` guards at
...
- [x] 1.3 Move the baseline build loop into `helper-baselines.R` (D7) and
...
- [x] 1.4 Promote the spec-delta placement check to a documented archive step
...
- [x] 2.1 Record the risk-set axis on the fit as documented, exported surface
...
- [x] 2.2 Specify `node_lookup` as the documented index resolver (D3) and close
...
- [x] 3.1 Move the six `*_default.cpp` engines' `event_scores` block onto
      ...
      this block.
- [x] 3.2 Re-verify the aggregate identity, which changes character rather than
      ...
      tautology.
- [x] 3.3 Re-run the three-way parity suite and record the movement. Expect it
...
- [x] 4.1 In-process guard that the reduction inputs are the likelihood inputs
...
- [x] 4.2 If 4.1 proves fragile in CI, degrade to documentation and **record
...
- [x] 5.1 Bisect the ~2.4e-07 relative drift of `fish_rem` / `fish_dynam_rate`
...
- [x] 5.2 **Spike, not a decision** — coordination rank ties (D10). The
...
- [x] 6.1 NEWS entry: the axis and node-lookup contract as what replaces reaching
      into internals (additive, not breaking — `margins` is untouched per task
      0.3), and, if section 3 lands, the per-event score as a stored-value change
      that moves no coefficient. DESCRIPTION version bump.
- [x] 6.2 Full `NOT_CRAN=true` suite green with the frozen baselines PASS (not
