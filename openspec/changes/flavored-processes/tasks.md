...
- [ ] 1.1 Write the factorized competing-flavor likelihood derivation as
...
- [ ] 1.2 Settle the intercept bookkeeping in the note: derive whether
...
- [ ] 1.3 Fold the note's conclusion back: amend the `flavored-processes` spec
...
- [ ] 2.1 Extend the layer-info metadata vocabulary with `flavor_style` and
...
- [ ] 2.2 Implement exported `add_flavor()` (thin: stamp `ties$flavor` from update
...
- [ ] 2.3 Tests for `add_flavor()` and the metadata validator (testthat 3e,
      hand-built stocnet fixtures; snapshot the cli abort messages under a pinned cli
      context)
- [ ] 2.4 Verification: full test run `NOT_CRAN=true` (baselines PASS not SKIP);
      commit

## 3. Multi-flavor specification surface

- [ ] 3.1 Lift the single-flavor abort in `make_specification()`: multi-key validation
...
- [ ] 3.2 Default flavor inference on unflavored layers (increment ±1, replace 1/0)
      with the `cli_inform` assumption message; abort paths for ambiguous encodings and
      non-matching keys
- [ ] 3.3 Derive per-flavor support constraints for `mutually_exclusive` layers
...
- [ ] 3.4 Specification print: nest each modeled flavor under the dependent layer with
      its formulas and derived/combined constraint (cli semantic elements; snapshot
      tests under a pinned cli context)
- [ ] 3.5 Tests: multi-key validation matrix, inference, derived-constraint
      compilation, print snapshots
- [ ] 3.6 Verification: `NOT_CRAN=true` run (PASS not SKIP), `devtools::document()`;
      version bump in DESCRIPTION + NEWS.md entry (specification surface milestone);
      commit

## 4. Single-pass multi-flavor preprocessing

- [ ] 4.1 Effect-union planning: deduplicate effects shared across flavors' formulas
      into one computed statistics set with per-flavor effect maps
- [ ] 4.2 Per-flavor writer routing in the recipe loop: dependent-vs-right-censored
...
- [ ] 4.3 Per-flavor intercept scalars (`n_dep_events`, `total_time`,
      `avg_active_actors` over the flavor's combined mask) per the task-1 note;
      per-flavor availability C-format buffers
- [ ] 4.4 Return shape: list of per-flavor `preprocessed.goldfish` objects, each
      passing the existing engine-readiness checks
- [ ] 4.5 Tests: two-flavor fixtures with hand-computed dependent/RC partitions,
      shared-effect single computation, per-flavor scalars against fixture values,
      empty-risk-set abort naming the flavor
- [ ] 4.6 Verification: `NOT_CRAN=true` run (frozen baselines PASS not SKIP —
      single-flavor and plain paths unchanged); commit

## 5. Per-flavor estimation and the sectioned result

- [ ] 5.1 Estimation loop over per-flavor preprocessed objects (existing engines
      unchanged); container result class holding flavor-named results (rate/choice
      nested for DyNAM)
- [ ] 5.2 Methods: `print()` with cli sections per flavor, `coef()`/`vcov()`/`logLik()`
      returning flavor-named components with summed total log-likelihood;
      `devtools::document()`
