# model-spec-descriptor Specification

## Purpose
Model behavior is a descriptor computed when a specification is constructed,
not a class hierarchy of model variants. This capability fixes what the
descriptor carries and who may read it, so preprocessing and estimation
dispatch on the behavior a model has rather than re-deriving it from the
model and sub-model names at each site.
## Requirements
