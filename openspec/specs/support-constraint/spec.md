...
The per-event opportunity list — supplied via `set_preprocessing_opt(opportunities_list =)`, a preprocessing option, not a data component — SHALL be deprecated following the lifecycle process (`lifecycle::deprecate_warn()` pointing to `support_constraint`, badge, NEWS entry). During
