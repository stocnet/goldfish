# the row-reduction message reads correctly [plain]
    ...
    Code
      ...
      atom_kinds = c(0L, 1L))
    Message
      i `support_constraint`: the dyadic atoms `tie(net)` and `alter(x)` are consumed on the sender axis by row-reduction — a sender is at risk iff it has at least one allowed, present receiver other than itself.
