...
test_that(
  "as.data.frame: replace applies last event per node within window",
  {
...
test_that(
  "as.data.frame: increment — multiple events to the same node are summed",
  {
    ...
    result <- as.data.frame(nodes, time = 20, envir = environment())
    expect_equal(result$score, c(7.0, 3.0),
...
test_that(
  "as.data.frame: startTime excludes events before the window",
  {
    ...
    result <- as.data.frame(nodes, time = 20, startTime = 5,
      envir = environment())
    expect_equal(result$score, 2.0,
...
test_that(
  "as.data.frame: no linked events returns object unchanged",
  {
...
test_that(
  "as.data.frame: mixed replace+increment is caught by link_events",
  {
    nodes <- make_test_nodes(1)
    ev_mixed <- data.frame(
      node = "Actor 1", time = 1,
      replace = 1.0, increment = 1.0
    )
    expect_error(
...
test_that(
  "as.matrix: replace — last event per dyad wins within window",
  {
    ...
    expect_equal(result_early["Actor 1", "Actor 2"], 1.0,
...
test_that(
  "as.matrix: increment — multiple events to the same dyad are summed",
  {
    net <- make_test_network(3)
    nodes <- make_test_nodes(3)
    ev <- data.frame(
    ...
    result <- as.matrix(net, time = 10, envir = environment())
    expect_equal(result["Actor 1", "Actor 2"], 7.0,
...
test_that(
  "as.matrix: undirected network — increment updates both directions",
  {
    ...
    result <- as.matrix(net, time = 10, envir = environment())
    expect_equal(result["Actor 1", "Actor 2"], 3.0)
    expect_equal(result["Actor 2", "Actor 1"], 3.0,
...
test_that(
  "as.matrix: startTime excludes events before the window",
  {
    ...
    result <- as.matrix(net, time = 20, startTime = 5,
      envir = environment())
    expect_equal(result["Actor 1", "Actor 2"], 1.0,
      label = "only event at t=10 is in [startTime=5, time=20)"
    )
  }
)
...
test_that(
  "as.matrix: no linked events returns initial matrix unchanged",
  {
...
test_that(
  "as.matrix: mixed replace+increment is caught by link_events",
  {
    net <- make_test_network(2)
    nodes <- make_test_nodes(2)
    ev_mixed <- data.frame(
      time = 1, sender = "Actor 1", receiver = "Actor 2",
      replace = 1.0, increment = 1.0
    )
    expect_error(
...
test_that(
  "as.matrix: Social_Evolution increment equals row-by-row reference",
  {
    skip_on_cran()
    ...
    expect_identical(result_new, net_ref,
