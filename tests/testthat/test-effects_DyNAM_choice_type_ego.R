...
test_that("DyNAM choice indeg(type = 'ego') equals the REM-derived expansion", {
  ...
  expect_equal(
    choice_ego$initialStats, rem_ego$initialStats,
    tolerance = 1e-6,
    label = "initial ego-perspective statistic matches REM"
  )
  expect_equal(
    ReducePreprocess(choice_ego), ReducePreprocess(rem_ego),
    tolerance = 1e-6
  )
})
...
test_that("DyNAM choice preserves the two-mode ego guard (mirrors REM)", {
  ...
  effect_fun <- function(weighted = FALSE, is_two_mode = TRUE,
                         transformer_fn = identity, type = "ego") {
    NULL
  }
  expect_error(
