...
test_that(
  "make_nodes functions as expected.",
  {
    ...
    expect_error(make_nodes(cbind(label= c("Club 1","Club 2","Club 3"), present = c(TRUE,TRUE,FALSE), clubSize = c(7,9,2))),
                 "expects objects of class \"data.frame\"", label = "Function should only accept data frames")
  }
)
...
test_that(
  "make_network functions as expected.",
  {
    ...
    expect_no_error(make_network(matrix = networkState, nodes = actorsEx,
                               directed = TRUE), message = "Function has unexpected error")
