test_that("NA data handled effectively in objects", {
...
test_that("NA data handled effectively when linking to nodes.", {
...
test_that("NA data handled effectively when linking to networks.", {
...
test_that("make_nodes functions as expected.", {
  expect_no_error(
    make_nodes(clubsEx),
...
test_that("make_network functions as expected.", {
  matrixTestForNodes <- cbind(
    label = c("Club 1", "Club 2", "Club 3"),
    present = c(TRUE, TRUE, FALSE),
    clubSize = c(7, 9, 2)
  )
  ...
  expect_error(
    make_network(matrix = networkState, nodes = matrixTestForNodes),
...
test_that("link_events works as expected.", {
  non_df_input <- array(1:8, dim = c(2, 2, 2))

  expect_error(
    link_events(x = networkState, change_events = eventsIncrement),
...
test_that("make_dependent_events works as expected.", {
  non_df_input <- array(1:8, dim = c(2, 2, 2))
  ...
  expect_error(
    make_dependent_events(events = eventsIncrement, nodes = non_df_input),
    "Invalid argument \"nodes\": this function expects objects of class \"data.frame\" or \"nodes.goldfish\".",
...
test_that("make_data works as expected", {
  expect_error(
    make_data(),
...
test_that("as.matrix.network.goldfish works as expected", {
  expect_equal(
