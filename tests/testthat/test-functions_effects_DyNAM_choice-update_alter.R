...
test_that("alter returns NULL if there is no change", {
  expect_null(
    update_DyNAM_choice_alter(
      testAttr$fishingSkill, node = 1, replace = 10, n1 = 8, n2 = 0
    )$changes
  )
  expect_null(
    update_DyNAM_choice_alter(
      testAttr$fishingSkill, node = 2, replace = NA, n1 = 8, n2 = 0
    )$changes,
...
test_that("alter returns correct attributes on update", {
  expect_equal(
    update_DyNAM_choice_alter(
      testAttr$fishingSkill, node = 1, replace = 1, n1 = 8, n2 = 0
    )$changes,
    cbind(node1 = 2:8, node2 = rep(1, 7), replace = rep(1, 7))
  )
  expect_equal(
    update_DyNAM_choice_alter(
      testAttr$fishingSkill, node = 1, replace = 0, n1 = 8, n2 = 0
    )$changes,
  ...
  expect_equal(
    update_DyNAM_choice_alter(
      testAttr$fishingSkill, node = 8, replace = 1, n1 = 8, n2 = 0
    )$changes,
