...
test_that("REM and DYNAM return the same result", {
  expect_equal(init_DyNAM_choice.cycle(effectFUN_closure, m1, 1, 5, 5), init_REM_choice.cycle(effectFUN_closure, m1, 1, 5, 5), label = " for init")
  expect_equal(update_DyNAM_choice_cycle(
    m,
    sender = 3, receiver = 1, replace = 1,
    cache = mCache
