...
test_that("cycle returns NULL if there is no change", {
  ...
  attr(m1, "last_update") <- c(sender = 0, receiver = 0, eventorder = 0)
  expect_null(
    update_DyNAM_choice_cycle(
      m1,
      sender = 5,
...
test_that("cycle recognises tie creation correctly", {
  ...
  attr(mCache, "last_update") <- c(sender = 3, receiver = 4, event_order = 0)
  expect_equal(
    update_DyNAM_choice_cycle(
      `[<-`(m1, 4, 5, 0),
      sender = 4,
...
test_that("REM and DYNAM return the same result", {
  expect_equal(
    init_DyNAM_choice.cycle(effectFUN_closure, m1, 1, 5, 5),
    init_REM_choice.cycle(effectFUN_closure, m1, 1, 5, 5),
    label = " for init"
  )
  expect_equal(
    update_DyNAM_choice_cycle(
