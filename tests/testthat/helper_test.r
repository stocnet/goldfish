...
networks_meta <- data.frame(
  ...
  nodeset_def = c(NA, NA), # node attribute indicating agency when two-mode, and active nodes for a one-mode
  # Event processing
...
effects_rate <- data.frame(
  ...
  args = I(list(NULL, NULL)), # List of arguments with defaults (or additional variables)
...
colnames(formulas_effects_rate) <- c(1, 2, 5) # fids
rownames(formulas_effects_rate) <- c(1, 2) # gids
formulas_effects_rate[1, c(1, 3)] <- 1 # outdeg(calls) in formula 1 and 5
formulas_effects_rate[2, c(2)] <- 1 # outdeg(friendship) in formula 2

formulas_effects_choice <- matrix(0, nrow = 7, ncol = 3)
colnames(formulas_effects_choice) <- c(3, 4, 6) # fids
rownames(formulas_effects_choice) <- c(3, 4, 5, 6, 7, 8, 9) # gids
