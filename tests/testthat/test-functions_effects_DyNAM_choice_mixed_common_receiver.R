test_that("mixed_common_receiver returns a valid object on update", {
  # error when null
  # Testing netUpdate = 1
  expect_type(update_DyNAM_choice_mixed_common_receiver(list(m, m1), 5, 3, 2, 1, m0), "list")
  # Testing netUpdate = 2
  expect_type(update_DyNAM_choice_mixed_common_receiver(list(m, m1), 5, 3, 2, 2, m0), "list")
})

test_that("mixed_common_receiver doesn't update when replace == oldValue", {
  # Testing netUpdate = 1
  expect_equal(update_DyNAM_choice_mixed_common_receiver(list(m, m1), 5, 3, 0, 1, m0)$cache, m0)
  # Testing netUpdate = 2
  expect_equal(update_DyNAM_choice_mixed_common_receiver(list(m, m1), 5, 3, 0, 2, m0)$cache, m0)
})

test_that("mixed_common_receiver doesn't update when sender == receiver", {
  expect_equal(update_DyNAM_choice_mixed_common_receiver(list(m, m1), 5, 5, 0, 2, m0)$cache, m0)
})
...
test_that("init mixed_common_receiver checks two-mode and dimensions", {
  # Error on two-mode
  params_two_mode <- function(is_two_mode = TRUE, transformer_fn = identity) {NULL}
  expect_error(init_DyNAM_choice.mixed_common_receiver(params_two_mode, list(m, m1), NULL, 5, 5), "must not use when is a two-mode network")
  # Successful init
  expect_type(init_DyNAM_choice.mixed_common_receiver(effectFUN, list(m, m1), NULL, 5, 5), "list")
})
...
test_that("mixed_common_receiver history = sequential: adding to net1 produces no new paths", {
  expect_null(
    update_DyNAM_choice_mixed_common_receiver(
      list(m, m1), 5, 3, 2, 1, m0, history = "sequential"
    )$changes,
...
test_that("mixed_common_receiver history = sequential: removal from net1 same as pooled", {
  expect_equal(
    update_DyNAM_choice_mixed_common_receiver(
      list(m, m1), 1, 2, 0, 1, m0, history = "sequential"
    ),
    update_DyNAM_choice_mixed_common_receiver(
      list(m, m1), 1, 2, 0, 1, m0
    ),
...
test_that("init mixed_common_receiver returns empty cache when history = sequential", {
  effectFUN_seq <- function(
    ...
    history = "sequential") {}
  expect_equal(
    init_DyNAM_choice.mixed_common_receiver(effectFUN_seq, list(m, m1), NULL, 5, 5)$cache,
