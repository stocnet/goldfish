...
#' @param oldValue numeric. Previous value of the tie (sign-normalised)
#' @param transformer_fn function applied to the updated counts before storing
#'   in `res$changes`
#'
#' @return Updated `res` list
#' @noRd
apply_two_path_update <- function(res, ids, replace, oldValue, transformer_fn) {
  if (length(ids) > 0) {
    replaceValues <- pmax(
      0L,
      replace - oldValue + res$cache[cbind(ids[, 1], ids[, 2])]
    )
