...
preprocess_emdynam_competition <- function(){
  ...
  for(i in 1:nChains){
    aux <- getChainSample(net1 = net1, net2 = net2, 
                          time1 = time1, time2 = time2, 
                          labels = actors$label,
                          seed = seedsGenerated[i])
    creation.events.sample[[i]] <- aux[aux$replace == 1,]
    deletion.events.sample[[i]] <- aux[aux$replace == 0,]
    eventsTot <- arrange(rbind(creation.events.sample[[i]],deletion.events.sample[[i]]), time)  
    
    supportConstrain[[i]] <- computeSupportConstrain(eventsTot, net1, actors$label)
    supportConstrain[[i]] <- lapply(formulasType, function(mt) {
      if (mt == "creation") supportConstrain[[i]]$creation
      else if (mt == "deletion") supportConstrain[[i]]$deletion
      else NULL
    })
  }
  proposalProbs <- rep(lfactorial(nrow(creation.events.sample[[1]])+nrow(deletion.events.sample[[1]])),nChains)
  ...
  if(is.null(initialParameters)){
    calcProbInit <- lapply(1:length(formulasType),
  ...
  clusterExport(cl, list(
    "getChainSampleFromModel", "getDyNAMChoices", "getDyNAMRates",
    "labelTorow", "computeSupportConstrain", "getChoiceProb", "getWaitingTime",
    "loadDataFast", "getData"
  ))
  clusterEvalQ(cl, {
    library(goldfish)
    library(matrixStats)
  ...
  clusterExport(cl, c("time1","time2","initialParameters","formulas","net1","actors",
                      "environmentsChains", "supportConstrain","fixedParameters",
                      "modelTypes","subModelTypes","statistics","calcProbInit"))
  
  
  chainType <- c()
  
  
  resP <- clusterApply(cl, seq_along(statistics), 
                       function(iChain) Map(
  ...
  logProbabilities <- t(sapply(resP,function(x) sapply(x,"[[", "logLikelihood")))
  ...
  if(stratified){
    ...
    sampleIndex <- sort(sample(1:nChains, size = nDraws, replace = T, prob = chainSampleP))
  }
  ...
  if(length(sampleIndexUnique) ==1 ){
    ...
    if(stratified){
      auxIndex <- stratified_resample(cumsum(chainSampleP[!(1:nChains %in% sampleIndexUnique)]/sum(chainSampleP[!(1:nChains %in% sampleIndexUnique)])))
    }else if(residual_samp){
      auxIndex <- residual_resample(chainSampleP[!(1:nChains %in% sampleIndexUnique)]/sum(chainSampleP[!(1:nChains %in% sampleIndexUnique)]))
    }else{
      auxIndex <- sample((1:nChains)[!(1:nChains %in% sampleIndexUnique)], size = nDraws/5, replace = T, prob = chainSampleP[!(1:nChains %in% sampleIndexUnique)])
    }
  ...
  res_sgd <- sgd_refactor(initialParameters,
                          fixedParameters,
                          nChains,
                          formulas,
                          formulasType,
  ...
  for(i in 1:length(newParameters)){
    cat(paste0("Model ",i,": ",paste(round(newParameters[[i]],2), collapse = " "), "\n"))
    cat(paste0("Model ",i,": ",paste(round(newStdErrors[[i]],2), collapse = " "), "\n"))}
  ...
  clusterExport(cl, c("seed","candidateEvents","time1","time2","initialParameters",
                      "formulas","net1","actors","formulasType", "environment_fb",
                      "subModelTypes"))
  clusterExport(cl, list(
    "getChainSampleFromModel2", "getChainSampleFromModel2MC"
  ))
  splitIndicesPerCore <- splitIndices(nChains, nCores)
  resAux <- clusterApply(cl, seq_along(splitIndicesPerCore),
  ...
  proposalProbs <- unlist(lapply(resAux,"[[","resProposalProb"),recursive = T)
  
  
  for(iChain in 1:nChains ){
    creation.events.sample[[iChain]] <- subset(resPermute[[iChain]], replace == 1)
    deletion.events.sample[[iChain]] <- subset(resPermute[[iChain]], replace == 0)
    supportConstrain[[iChain]] <- computeSupportConstrain(resPermute[[iChain]], net1, actors$label)
    supportConstrain[[iChain]] <- lapply(formulasType, function(mt) {
      if (mt == "creation") supportConstrain[[iChain]]$creation
      else if (mt == "deletion") supportConstrain[[iChain]]$deletion
      else NULL
    })
  }
  ...
  clusterExport(cl, c("statistics","subModelTypes","calcProbInit",
                      "environmentsChains","supportConstrain"))
  resP <- clusterApply(cl, seq_along(statistics), 
                       function(iChain) Map(
                         function(formula, sub_model, preproc_init, op_list, control_est) {
                           estimate_dynam(
  ...
  logLikelihoods <- t(sapply(resP, function(x) sapply(x, function(model) model$logLikelihood)))
