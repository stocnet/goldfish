...
update_DyNAM_rate_indeg <- function(
  ...
  changes <- cbind(node1 = receiver, replace = forceAndCall(1, transformer_fn, cache[receiver]))
...
update_DyNAM_rate_outdeg <- function(
  ...
  changes <- cbind(node1 = sender, replace = forceAndCall(1, transformer_fn, cache[sender]))
  list(cache = cache, changes = changes)
}
...
init_DyNAM_rate.node_trans <- function(effectFun, network, window, n1, n2, ...) {
  ...
  if (is_two_mode) {
    stop("'node_trans' effect must not be used when is a two-mode network",
      call. = FALSE
    )
  }
  if ((!is.null(window) && !is.infinite(window)) || all(network == 0)) {
...
init_DyNAM_rate.tertius <- function(
  ...
  if (is_two_mode) {
    stop("'tertius' effect must not be used with type 'ego' when is a two-mode network",
      call. = FALSE
    )
  }
  if ((!is.null(window) && !is.infinite(window)) || all(network == 0)) {
...
update_DyNAM_rate_tertius <- function(
  ...
  if (!is.null(node) && is.null(sender) && is.null(receiver)) {
    ...
    cache[outNode] <- vapply(X = outNode, FUN = function(x) {
...
init_DyNAM_rate.ego <- function(effectFun, attribute, n1, n2, ...) {
  ...
  is_two_mode <- eval(params[["is_two_mode"]])
  if (is_two_mode) {
    stop("'ego' effect must not be used with type 'ego' in a two-mode network",
      call. = FALSE
    )
  }
  list(stat = attribute)
...
update_DyNAM_rate_ego <- function(
  ...
  if (attribute[node] == replace) return(res)
...
init_DyNAM_rate.global <- function(effectFun, attribute, n1, n2, ...) {
  if (!inherits(attribute, "global.goldfish")) {
    stop("'global' effect requires an object of class 'global.goldfish' created by make_global_attribute()",
      call. = FALSE
    )
  }
  current_val <- attribute$replace[nrow(attribute)]
  list(stat = rep(current_val, n1))
}
...
update_DyNAM_rate_global <- function(
    ...
    node = NULL, replace,
    n1, n2, ...) {
  if (is.null(replace)) return(list(changes = NULL))
  changes <- cbind(node1 = seq_len(n1), replace = replace)
