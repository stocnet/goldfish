...
#' @noRd
#'
estimate_emdynam <- function(panel_data,
                             rem_data,
                             actors,
                             rem_actors = NULL,
                             formulas,
  ...
  while(iStep <= nSteps){
    ...
    newCalcProbInit <- lapply(1:length(newParameters), function(x) set_estimation_opt(
    ...
    newlogProbabilities <- t(sapply(resP, function(x) sapply(x, function(model) model$logLikelihood)))
    ...
    Lambda <- (rowSums(newlogProbabilities[sampleIndexUnique,])-rowSums(logProbabilities[sampleIndexUnique,]))
    Q <- sum(as.vector(sampleTable)*Lambda)/nDraws
    sigma2 <- Q^2 * ( sum(as.vector(sampleTable)*(Lambda)^2 )/
    ...
    save(newlogProbabilities,newParameters,newStdErrors,nChains,logProbabilities,
         sampleTable,initialParameters,initialStdErrors,iStep,Q,ASE,chainSampleP,file = filename)
    ...
    if(abs(Q+qnorm(1-gamma)*ASE) < epsEnd  & (Q-qnorm(1-alpha)*ASE) > 0) {
      cat(paste0("end","\n"))
      stopIt <- stopIt + 1
      # break
    }
    ...
    if((Q-qnorm(1-alpha)*ASE)<0){
      ...
      for(i in 1:nSimChains ){
        ...
        creation.events.sample[[iChain]] <- subset(resPermute[[i]], replace == 1)
        deletion.events.sample[[iChain]] <- subset(resPermute[[i]], replace == 0)
        supportConstrain[[iChain]] <- computeSupportConstrain(resPermute[[i]], net1, actors$label)
      ...
      environmentsChains <- c(environmentsChains, lapply((nChains+1):(nChains+nSimChains), loadDataFast))  
      ...
      clusterExport(cl, c("statistics","subModelTypes","calcProbInit",
                          "environmentsChains","nChains","supportConstrain"))
      resP <- clusterApply(cl, 1:nSimChains, 
                           function(iChain) Map(
                             function(formula, sub_model, preproc_init, op_list, control_est) {
                               estimate_dynam(
                                 x = formula,
      ...
      nSimChains = nChains - ceiling(max(nChains,sigma2*(qnorm(1-beta)+qnorm(1-alpha))^2/Q^2)) 
  ...
  retunr(list("parameters" = newParameters, "std.errors" = newStdErrors,
