...
List estimate_REM(const arma::vec& parameters,
    ...
    for (int id_event = 0; id_event < n_events; id_event++) {
            ...
            if (presence1(i) == 1) {
                ...
                for (int j = 0; j < n_actors_2; j++) {
                    if (presence2(j) == 1 && (j != not_allowed_receiver)) {
                        ...
                        weighted_sum_current_event += exp_current_receiver * (current_data_matrix.row(j));
        ...
        // update id_dep_event
        if (is_dependent(id_event)) {
            intervalLogL(id_event) += dot(stat_mat.row(id_sender * n_actors_2 + id_receiver), parameters);
