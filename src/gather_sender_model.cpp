...
List gather_sender_model(const arma::mat& dep_event_mat,
    ...
    int size_stat_all_events = n_actors_1 * 20;
    ...
    for (int id_event = 0; id_event < n_events; id_event++) {
        ...
        if (is_dependent(id_event)) {
            while (stat_mat_update_id < stat_mat_update_pointer(id_dep_event)) {
                stat_mat(stat_mat_update(0, stat_mat_update_id)*n_actors_2 + stat_mat_update(1, stat_mat_update_id), stat_mat_update(2, stat_mat_update_id)) = stat_mat_update(3, stat_mat_update_id);
                stat_mat_update_id++;
            }
        } else {
            while (stat_mat_rightcensored_update_id < stat_mat_rightcensored_update_pointer(id_event - id_dep_event)) {
                stat_mat(stat_mat_rightcensored_update(0, stat_mat_rightcensored_update_id)*n_actors_2 + stat_mat_rightcensored_update(1, stat_mat_rightcensored_update_id), stat_mat_rightcensored_update(2, stat_mat_rightcensored_update_id)) = stat_mat_rightcensored_update(3, stat_mat_rightcensored_update_id);
        ...
        // reduce the stat mat from n_actors_1 * n_actors_2 * n_parameters to n_actors_1 * n_parameters by taking average.
        arma::mat reduced_stat_mat = reduce_mat_to_vector(stat_mat, n_actors_1, n_actors_2, twomode_or_reflexive);
        // declare the ids of the sender and the receiver, and initialize the variable to count the present pairs of actors in this event.
        int n_present = 0;
